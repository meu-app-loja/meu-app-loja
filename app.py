...
# ==============================================================================
# 🕒 AJUSTE DE FUSO HORÁRIO E FUNÇÕES DE BUSCA
# ==============================================================================
...
def parse_num_br(x, default=0.0):
    """Converte números vindos de Excel/CSV (pt-BR ou en-US) para float."""
    try:
...
def ler_excel_com_header_auto(file_obj, max_rows=25):
    ...
    # Lendo como str para não destruir códigos
...
# ==============================================================================
# 🛡️ BLINDAGEM: conversão numérica pt-BR + clamp de negativos
# ==============================================================================
def _to_float(valor):
    try:
...
def carregar_historico(prefixo_arquivo):
    try:
        ...
        # Garante colunas de código de barras no histórico
...
if df is not None:
                ...
                if st.button("✅ CONFIRMAR IMPORTAÇÃO"):
                    ...
                    for i, item in enumerate(dados['itens']):
                        ...
                        if "[SISTEMA]" in esc:
                             ...
                             if len(parts) > 1:
                                 cod_barras_hist = parts[0]
                             nome_final = parts[1] if len(parts) > 1 else raw_sel
                        else:
    ...
    # CIRURGIA 2: Foge da coluna "Qtd.Padrão" que pegava o valor 6 da Água Crystal
        ...
        if arquivo:
            ...
            if not df_raw.empty:
                ...
                # A MÁGICA ESTÁ AQUI: Só aceita coluna "qtd" se ELA NÃO TIVER a palavra "padr" (Padrão)
        ...
        if df.empty:
            ...
            if usar_modo_mobile:
                ...
                with tab_acao:
                    # CIRURGIA GÔNDOLA: Assegurando que a data de validade não se misture no display de busca
        ...
        # 🧑‍⚕️ CIRURGIA: NOVO BOTÃO DE RESET PARA LIMPAR O BANCO DE DADOS BUGADO!
        st.markdown("### 🧨 RESET TOTAL (Apagar Banco de Dados)")
        st.write("Se os seus produtos ficaram com nomes estranhos ou datas no lugar de códigos (por erro de mapeamento do Planograma), clique aqui para apagar tudo e importar do zero.")
        if st.button("🗑️ APAGAR TODOS OS PRODUTOS", type="primary"):
            df.drop(df.index, inplace=True)
            salvar_estoque(df, prefixo)
            st.success("✅ Banco de dados apagado! Vá em 'Sincronizar (Planograma)' e importe a planilha novamente prestando atenção nas colunas.")
            time.sleep(3)
            st.rerun()
        ...
        st.divider()
        # 🧑‍⚕️ CIRURGIA: MÁQUINA DO TEMPO COM INTELIGÊNCIA ARTIFICIAL!
        st.markdown("### 🪄 Máquina do Tempo: Reconstruir Estoque da Casa")
        st.write("Se a sua Casa está zerada porque o nome das notas fiscais antigas não bateram com o seu cadastro novo, **use esta ferramenta**. A inteligência do sistema vai associar 'Coca 2Lt' com 'Coca Cola Tradicional 2L' e jogar o estoque no lugar certo!")
        ...
        if st.button("🚀 RECONSTRUIR CASA AGORA (Baseado no Histórico)", type="primary"):
            with st.spinner("Lendo todos os históricos e usando Inteligência de Busca para repor a Casa..."):
                ...
                # 2. Injeta as Compras
                if not df_c.empty:
                    lista_nomes_sistema = df['nome do produto'].tolist()
                    for _, row_c in df_c.iterrows():
                        ...
                        # Se não achou pelo código (ou tava vazio), usa a BUSCA APROXIMADA PELO NOME!
                        if idx_alvo is None and nome_hist:
                            melhor_nome, status_match = encontrar_melhor_match(nome_hist, lista_nomes_sistema, cutoff=0.35)
                            if melhor_nome:
                ...
                for idx, row in df.iterrows():
                    ...
                    if not df_v.empty:
                        mask_v_cod = df_v['código_barras'].apply(padronizar_codigo_barras) == cod_limpo
                        mask_v_nome = df_v['produto'].astype(str).str.upper() == nome_sis
                        mask_v = mask_v_cod | mask_v_nome if cod_limpo else mask_v_nome
                        qtd_venda = float(df_v[mask_v]['qtd_vendida'].sum())
                        
                    saldo_casa = df.at[idx, 'qtd_central'] - qtd_venda - loja_atual
                ...
                salvar_estoque(df, prefixo)
                st.success(f"✅ Sucesso! {count_rec} produtos tiveram o estoque da Casa recalculado e restaurado com Inteligência Artificial.")
                st.balloons()
                time.sleep(3)
                st.rerun()
