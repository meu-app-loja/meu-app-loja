...
def salvar_na_nuvem(nome_aba, df, colunas_padrao):
    try:
        ...
        st.warning(f"Tentando salvar novamente... ({e})")
        time.sleep(2)
        try:
            ws.update([df_save.columns.values.tolist()] + df_save.values.tolist())
        except: st.error(f"Erro persistente ao salvar: {e}")
...
# --- NOVA FUNÇÃO DE PROPAGAÇÃO EM MASSA (SUBSTITUI ATUALIZAÇÕES INDIVIDUAIS EM LOOPS) ---
def propagar_dados_massa(df_referencia, prefixo_ignorar):
    """
    Atualiza TODAS as outras lojas de uma só vez, evitando erro 429 do Google.
    Usa o df_referencia como a 'verdade' para Qtd Central, Preço Custo, Preço Venda.
    """
    status_box = st.status("🔄 Sincronizando lojas em massa (Modo Turbo)...", expanded=True)
    todas_lojas = ["loja1", "loja2", "loja3"]
    client = get_google_client()
    sh = client.open("loja_dados")
    
    # Mapa de referência para busca rápida
    ref_map = {}
    for _, row in df_referencia.iterrows():
        ...
        ref_map[nome] = {
            ...
            'validade': str(row['validade']).replace('NaT', '') if pd.notnull(row['validade']) else '',
    ...
    for loja in todas_lojas:
        ...
        status_box.write(f"Conectando a {loja}...")
        try:
            # LER UMA VEZ
            ...
            # Limpa colunas para garantir match
            ...
            for i, row_d in df_dest.iterrows():
                ...
                if nome_d in ref_map:
                    ...
                    # Verifica diferenças (com margem para float)
                    atual_qtd = converter_numero_seguro(row_d['qtd_central'])
                    atual_custo = converter_numero_seguro(row_d['preco_custo'])
                    
                    if abs(atual_qtd - novos['qtd_central']) > 0.01:
                        df_dest.at[i, 'qtd_central'] = novos['qtd_central']; alterou = True
                    if abs(atual_custo - novos['custo']) > 0.01:
                        df_dest.at[i, 'preco_custo'] = novos['custo']; alterou = True
                    # Se tiver preço de venda definido na origem, copia
                    if novos['venda'] > 0:
                        if abs(converter_numero_seguro(row_d['preco_venda']) - novos['venda']) > 0.01:
                            df_dest.at[i, 'preco_venda'] = novos['venda']; alterou = True
                    if novos['forn']:
                        df_dest.at[i, 'ultimo_fornecedor'] = novos['forn']
            
            # SALVAR UMA VEZ
            if alterou:
                status_box.write(f"Salvando atualizações em {loja}...")
                # Garante formatação antes de subir
            ...
            status_box.write(f"⚠️ Aviso em {loja}: {e}")
            
    status_box.update(label="✅ Sincronização Concluída!", state="complete", expanded=False)

# --- FUNÇÃO INDIVIDUAL (PARA GÔNDOLA - MANTIDA PARA PEQUENOS AJUSTES) ---
        ...
        if arquivo:
            try:
                ...
                if st.button("🚀 SINCRONIZAR"):
                    ...
                    # PROPAGAÇÃO OTIMIZADA AQUI (SUBSTITUIU LOOP LENTO)
    ...
    # 8. ESTOQUE CENTRAL
        ...
        with tab1:
            ...
            if st.button("💾 Salvar"):
                ...
                propagar_dados_massa(df, prefixo) # OTIMIZADO
