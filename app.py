...
import json
import time
import locale  # CORREÇÃO: Import para handling de locale BR em números
    ...
    try:
        ...
        for col in df.columns:
            ...
            if any(x in c_low for x in ['qtd', 'preco', 'valor', 'custo', 'total', 'desconto']):
                df[col] = df[col].astype(str).apply(converter_numero_seguro)  # CORREÇÃO: Força string antes de converter para evitar perda de decimal
    ...
    try:
        ...
        for col in df_save.columns:
            ...
            if any(x in c_low for x in ['qtd', 'preco', 'valor', 'custo', 'total', 'desconto']):
                 ...
                 df_save[col] = df_save[col].apply(lambda x: f"{x:.2f}" if pd.notnull(x) else '')  # CORREÇÃO: Formata como string com 2 decimais (ponto) para Sheets preservar
...
def processar_excel_oficial(arquivo_subido):
    try:
        ...
        if arquivo_subido.name.endswith('.csv'):
            df_temp = pd.read_csv(arquivo_subido, dtype=str, thousands='.', decimal=',')  # CORREÇÃO: Parâmetros para locale BR
        else:
    ...
    if arquivo:
        try:
            ...
            if arquivo.name.endswith('.csv'):
                df_raw = pd.read_csv(arquivo, header=None, dtype=str, thousands='.', decimal=',')  # CORREÇÃO: Parâmetros para locale BR
            else:
    ...
    with tab_imp:
        ...
        if arquivo_vendas:
            try:
                ...
                df_vendas_temp = pd.read_excel(arquivo_vendas, header=linha_titulo, dtype=str, thousands='.', decimal=',')  # CORREÇÃO: Locale BR
