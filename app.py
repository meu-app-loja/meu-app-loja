...
def converter_ptbr(valor):
    """Converte valores brasileiros (com vírgula) para padrão float - VERSÃO CORRIGIDA"""
    ...
    # Lógica corrigida para 3,19 não virar 319
    if ',' in s and '.' in s:
        pos_virgula = s.rfind(',')
        pos_ponto = s.rfind('.')
        ...
        if pos_virgula > pos_ponto:
            ...
            # Padrão US: 1,234.56 -> remove vírgula de milhar
            s = s.replace(',', '')
    elif ',' in s:
        # Só tem vírgula: assume como decimal BR (3,19 -> 3.19)
        s = s.replace(',', '.')
...
# --- SALVAR NA NUVEM (CORRIGIDA COM PROTEÇÃO) ---
def salvar_na_nuvem(nome_aba, df, colunas_padrao):
    """Salva com proteção contra perda de dados"""
    try:
        client = get_google_client()
        sh = client.open("loja_dados")
        try: 
        ...
        # IMPORTANTE: Valida antes de limpar
        df_save = garantir_integridade_colunas(df.copy(), colunas_padrao)
        
        # Valida que o DataFrame não está vazio
        if df_save.empty:
            st.warning(f"⚠️ Tentativa de salvar DataFrame vazio em {nome_aba} - operação cancelada!")
            return
        ...
        # Garante formato numérico correto para preços
        for col in df_save.columns:
            if any(x in col for x in ['preco', 'custo', 'total', 'desconto']):
                df_save[col] = df_save[col].apply(lambda x: float(x) if pd.notnull(x) else 0.0)
        
        # Só limpa DEPOIS de validar
        ws.clear()
...
def atualizar_casa_global(nome_produto, qtd_nova_casa, novo_custo, novo_venda, nova_validade, prefixo_ignorar):
    """Atualiza estoque central em todas as lojas - CORRIGIDO"""
    ...
    for loja in todas_lojas:
        ...
        try:
            ...
            st.warning(f"Aviso ao atualizar {loja}: {e}")
                ...
                if df.empty: 
                    ...
                    if produtos_baixo.empty: 
                        ...
                        if novos_itens:
                            ...
                            st.warning("Itens já estão na lista.")
            st.divider()
                ...
                if not novo_cod or not novo_nome: 
                    ...
                    try:
                        novo = {
                            ...
                            'qtd.estoque': float(ini_loja), 
                            'qtd_central': float(ini_casa), 
                            'qtd_minima': float(novo_min), 
                            'validade': pd.to_datetime(ini_val) if ini_val else None, 
                            'status_compra': 'OK', 
                            'qtd_comprada': 0.0, 
                            'preco_custo': float(novo_custo), 
                            'preco_venda': float(novo_venda), 
                            'categoria': nova_cat, 
                            'ultimo_fornecedor': '', 
            ...
            try:
                ...
                if st.button("✅ CONFIRMAR E SALVAR ESTOQUE"):
                    ...
                    for i, item in enumerate(dados['itens']):
                        ...
                        if produto_escolhido == "(CRIAR NOVO)":
                            novo_prod = {
                                ...
                                'qtd.estoque': 0.0, 
                                'qtd_central': float(qtd_xml), 
                                'qtd_minima': 5.0, 
                                'validade': None, 
                                'status_compra': 'OK', 
                                'qtd_comprada': 0.0, 
                                'preco_custo': float(preco_pago), 
                                'preco_venda': float(preco_pago * 2), 
                                'categoria': 'GERAL', 
                                'ultimo_fornecedor': dados['fornecedor'], 
                                'preco_sem_desconto': float(preco_sem_desc)
                            }
                            ...
                            if mask.any():
                                idx = df[mask].index[0]
                                df.at[idx, 'qtd_central'] = float(df.at[idx, 'qtd_central']) + float(qtd_xml)
                                df.at[idx, 'preco_custo'] = float(preco_pago)
                                df.at[idx, 'preco_sem_desconto'] = float(preco_sem_desc)
                                df.at[idx, 'ultimo_fornecedor'] = dados['fornecedor']
                                atualizados_cont += 1
                                nome_final = produto_escolhido
                        
                        # CORREÇÃO CRÍTICA: Sincroniza com todas as lojas
                        if nome_final:
                            qtd_central_atual = df.loc[df['nome do produto'] == nome_final, 'qtd_central'].values[0]
                            atualizar_casa_global(
                                nome_final, 
                                float(qtd_central_atual), 
                                float(preco_pago), 
                                None, 
                                None, 
                                prefixo
                    ...
                    try:
                        ...
                        st.success("✅ Sincronizado em TODOS os menus (Tabela Geral, Estoque, Gôndola)!")
                        st.balloons()
                        st.rerun()
                            ...
                            with st.form("form_transf_gondola"):
                                ...
                                if st.form_submit_button("⬇️ CONFIRMAR TRANSFERÊNCIA"):
                                    if qtd_transf > 0:
                                        ...
                                        st.info("Quantidade deve ser maior que 0.")
                        ...
                        with st.form("edit_estoque_casa_full"):
                            ...
                            if st.form_submit_button("💾 SALVAR"):
                                ...
                                if acao.startswith("Somar") and qtd_input > 0:
                                    ...
                                    hist = {'data': dt_full, 'produto': c_nome.upper().strip(), 'fornecedor': c_forn, 'qtd': qtd_input, 'preco_pago': novo_custo, 'total_gasto': qtd_input * novo_custo, 'numero_nota': '', 'desconto_total_money': 0, 'preco_sem_desconto': 0}
                                    df_hist = pd.concat([df_hist, pd.DataFrame([hist])], ignore_index=True)
                                    salvar_na_nuvem(f"{prefixo}_historico_compras", df_hist, COLS_HIST)
