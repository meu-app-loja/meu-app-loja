...
# Isso garante que o sistema nunca trave, mesmo se a planilha estiver vazia
...
def garantir_integridade_colunas(df, colunas_alvo):
    ...
    # Verifica se falta algo vital e cria se necessário
...
def ler_da_nuvem(nome_aba, colunas_padrao):
    time.sleep(1.0) # Pausa para não bloquear o Google
    try:
        ...
        # Aplica a vacina contra erros de coluna
...
def salvar_na_nuvem(nome_aba, df, colunas_padrao):
    try:
        ...
        df_save = df.copy()
        # Garante integridade antes de salvar
        df_save = garantir_integridade_colunas(df_save, colunas_padrao)
...
# ==============================================================================
# 🧠 SUAS FUNÇÕES ORIGINAIS (MANTIDAS 100% E MOVIDAS PARA O TOPO)
# ==============================================================================
...
def unificar_produtos_por_codigo(df):
    ...
    df = garantir_integridade_colunas(df, COLS_ESTOQUE)
    ...
    for loja in todas_lojas:
        ...
        if not df_outra.empty:
            ...
            if mask.any():
                ...
                salvar_na_nuvem(f"{loja}_estoque", df_outra, COLS_ESTOQUE)

# --- FUNÇÃO XML INTELIGENTE HÍBRIDA (ADAPTADA) ---
def ler_xml_nfe(arquivo_xml, df_referencia):
    ...
    # 2. TENTA FORMATO PADRÃO NFE (SEFAZ)
    else:
...
st.sidebar.markdown("---")
# Mantive value=False pois você disse que está no notebook
...
# --- CARREGAMENTO INICIAL COM SEGURANÇA ---
            ...
            try:
                ...
                if not col_barras or not col_qtd: st.error("❌ Colunas 'Código de Barras' ou 'Transferir' não encontradas.")
                    ...
                    if st.button("🚀 PROCESSAR TRANSFERÊNCIA"):
                        ...
                        salvar_na_nuvem(f"{prefixo}_estoque", df, COLS_ESTOQUE)
                        if log_movs:
        ...
        if arquivo:
            try:
                ...
                if st.button("🚀 SINCRONIZAR TUDO (Importar + Atualizar)"):
                    df = ler_da_nuvem(f"{prefixo}_estoque", COLS_ESTOQUE) # Recarrega
    ...
    # 7. HISTÓRICO & PREÇOS
        ...
        if not df_hist.empty:
            ...
            if st.button("💾 Salvar Alterações e Exclusões"):
                ...
                salvar_na_nuvem(f"{prefixo}_historico_compras", df_hist, COLS_HIST)
                st.success("Histórico salvo e sincronizado!"); st.rerun()
            ...
            with c1:
                if st.button("💾 SALVAR ALTERAÇÕES GERAIS"):
                    ...
                    for i, (idx, row) in enumerate(df_edit.iterrows()):
                        ...
                        bar.progress((i+1)/total)
                    st.success("Tabela Geral atualizada e sincronizada!"); st.rerun()
            with c2:
