...
if modo == "📊 Dashboard (Visão Geral)":
    ...
    if arquivo:
        try:
            if arquivo.name.endswith('.csv'): df_raw = pd.read_csv(arquivo, header=None, dtype=str, decimal=',', thousands='.')
            else: df_raw = pd.read_excel(arquivo, header=None, dtype=str, decimal=',', thousands='.')
