...
# Colunas que OBRIGATORIAMENTE precisam existir para o seu código original rodar
# Se o planograma apagar, o sistema recria essas colunas automaticamente.
COLUNAS_VITAIS = [
...
def garantir_integridade_colunas(df):
    ...
    # Verifica se falta algo vital
    for col in COLUNAS_VITAIS:
        if col not in df.columns:
            # Recria a coluna faltando com 0 ou vazio
...
def ler_da_nuvem(nome_aba):
    ...
    try:
        ...
        # SE FOR ESTOQUE, APLICA A VACINA CONTRA ERROS
        if "estoque" in nome_aba:
            df = garantir_integridade_colunas(df)
            
        # Converte tipos para o seu código não reclamar
        ...
        return df
    except: return pd.DataFrame(columns=COLUNAS_VITAIS if "estoque" in nome_aba else [])
...
def salvar_na_nuvem(nome_aba, df):
    try:
        ...
        df_save = df.copy()
        # Garante integridade antes de salvar também
        if "estoque" in nome_aba:
            df_save = garantir_integridade_colunas(df_save)
            
        # Prepara datas para o Google
        for col in df_save.columns:
            if pd.api.types.is_datetime64_any_dtype(df_save[col]):
                df_save[col] = df_save[col].astype(str).replace('NaT', '')
                
        ws.update([df_save.columns.values.tolist()] + df_save.values.tolist())
        ler_da_nuvem.clear() # Limpa cache para ver atualização imediata
...
def unificar_produtos_por_codigo(df):
    ...
    df = garantir_integridade_colunas(df) # Proteção extra
...
def processar_excel_oficial(arquivo_subido):
    try:
        ...
        # Salva na nuvem em vez de local
...
# Esta função substitui a edição direta de arquivos. Agora ela chama a nuvem.
...
# --- FUNÇÃO XML INTELIGENTE (ADAPTADA PARA LER DE DF EM VEZ DE ARQUIVO) ---
...
# ==============================================================================
# 🚀 INÍCIO DO APP (AGORA SIM, IGUAL AO ORIGINAL)
# ==============================================================================
...
if df is not None: # Mantive a estrutura do seu 'if df is not None'
        ...
        with tab_lista:
            if not df_lista_compras.empty:
                ...
                if c_del.button("🗑️ Limpar Lista Inteira (Após Comprar)"):
                    # Salva lista vazia, mas com colunas certas
                ...
                if df.empty: st.warning("Sem produtos cadastrados.")
                    ...
                    if produtos_baixo.empty: st.success("Tudo certo! Nada abaixo do mínimo.")
                        ...
                        if novos_itens:
                            ...
                            salvar_na_nuvem(f"{prefixo}_lista_compras", df_lista_compras)
        ...
        if arquivo_xml:
            try:
                ...
                for i, item in enumerate(dados['itens']):
                    ...
                    qtd_xml = item['qtd']
                    p_bruto = item['preco_un_bruto']; p_liq = item['preco_un_liquido']; desc_total = item['desconto_total_item']
                ...
                if st.button("✅ CONFIRMAR E SALVAR ESTOQUE"):
                    ...
                    for i, item in enumerate(dados['itens']):
                        ...
                        qtd_xml = int(item['qtd']); preco_pago = item['preco_un_liquido']; preco_sem_desc = item['preco_un_bruto']; desc_total_val = item['desconto_total_item']
                        ...
                        novos_hist.append({'data': datetime.now(), 'produto': nome_final, 'fornecedor': dados['fornecedor'], 'qtd': qtd_xml, 'preco_pago': preco_pago, 'total_gasto': qtd_xml * preco_pago, 'numero_nota': dados['numero'], 'desconto_total_money': desc_total_val, 'preco_sem_desconto': preco_sem_desc})
                    salvar_na_nuvem(f"{prefixo}_estoque", df)
                    if novos_hist:
                        df_hist = pd.concat([df_hist, pd.DataFrame(novos_hist)], ignore_index=True)
                        salvar_na_nuvem(f"{prefixo}_historico_compras", df_hist)
        ...
        if arquivo:
            try:
                ...
                if st.button("🚀 SINCRONIZAR TUDO (Importar + Atualizar)"):
                    df = ler_da_nuvem(f"{prefixo}_estoque") # Recarrega para garantir
    ...
    # 7. HISTÓRICO & PREÇOS
        ...
        if not df_hist.empty:
            ...
            df_editado = st.data_editor(df_hist_visual.sort_values(by='data', ascending=False), use_container_width=True, key="editor_historico_geral", num_rows="dynamic", column_config={"preco_sem_desconto": st.column_config.NumberColumn("Preço Tabela", format="R$ %.2f"), "desconto_total_money": st.column_config.NumberColumn("Desconto TOTAL", format="R$ %.2f"), "preco_pago": st.column_config.NumberColumn("Pago (Unit)", format="R$ %.2f", disabled=True), "total_gasto": st.column_config.NumberColumn("Total Gasto", format="R$ %.2f", disabled=True)})
            if st.button("💾 Salvar Alterações e Exclusões"):
                ...
                for idx, row in df_hist.iterrows():
                    try:
                        q = float(row.get('qtd', 0)); p_tab = float(row.get('preco_sem_desconto', 0)); d_tot = float(row.get('desconto_total_money', 0))
                        if q > 0 and p_tab > 0:
