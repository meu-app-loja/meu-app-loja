...
# --- FUNÇÃO DE LIMPEZA MATEMÁTICA ABSOLUTA V4 (CORRIGIDA) ---
def converter_numero_seguro(valor):
    """
    Converte 3,19 ou 3.19 para 3.19. Evita o erro de 319.0
    """
    ...
    try:
        # Se tem ponto E vírgula (Ex: 1.200,50), o ponto é milhar
        if '.' in s_valor and ',' in s_valor:
            s_valor = s_valor.replace('.', '').replace(',', '.')
        
        # Se tem APENAS vírgula (Ex: 3,19), a vírgula é decimal
        elif ',' in s_valor:
            s_valor = s_valor.replace(',', '.')
            
        # Se tem APENAS ponto (Ex: 3.19), já está certo (Python usa ponto)
...
def processar_excel_oficial(arquivo_subido):
    try:
        # CORREÇÃO: dtype=str para ler numeros como texto e nao perder o ponto
...
# --- ATUALIZAR CASA GLOBAL (COM PAUSA MAIOR PARA NÃO TRAVAR) ---
def atualizar_casa_global(nome_produto, qtd_nova_casa, novo_custo, novo_venda, nova_validade, prefixo_ignorar):
    todas_lojas = ["loja1", "loja2", "loja3"]
    time.sleep(2) # PAUSA DE 2 SEGUNDOS PARA NÃO DAR ERRO 429
...
if df is not None:
            ...
            try:
                ...
                if st.button("✅ CONFIRMAR E SALVAR ESTOQUE"):
                    ...
                    for i, item in enumerate(dados['itens']):
                        ...
                        # PROTEÇÃO DE ERRO: Tenta sincronizar, mas se falhar, continua para salvar a loja atual
                        try:
    ...
    # 9. GERAL
        ...
        if c1.button("💾 Salvar"):
            ...
            st.success("Salvo!")
        with c2:
            # FERRAMENTA DE CORREÇÃO FLEXÍVEL ADICIONADA AQUI
