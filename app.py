...
def converter_numero_seguro(valor):
    """Garante que 3.19 continue 3.19"""
    if pd.isna(valor) or str(valor).strip() == "": return 0.0
    s_valor = str(valor).strip().replace('R$', '').replace('r$', '').strip()
    try:
        if '.' in s_valor and ',' in s_valor: s_valor = s_valor.replace('.', '').replace(',', '.')
        elif ',' in s_valor: s_valor = s_valor.replace(',', '.')
...
def processar_excel_oficial(arquivo_subido):
    try:
        # CORREÇÃO: dtype=str para ler o 3.19 corretamente
...
def atualizar_casa_global(nome_produto, qtd_nova_casa, novo_custo, novo_venda, nova_validade, prefixo_ignorar):
    ...
    time.sleep(1.5) # Pausa estratégica anti-travamento
    ...
    for item in dados_nota['itens']:
        if item['ean'] in ['SEM GTIN', '', 'None', 'NAN'] and lista_nomes_ref:
            ...
            if melhor: 
                ...
                item['match_tipo'] = tipo # Guarda o tipo para exibir
    return dados_nota
            ...
            if arquivo_vendas:
                try:
                    ...
                    if st.button("🚀 PROCESSAR VENDAS"):
                        at = 0; regs = []; bar = st.progress(0); tot = len(df_vendas_temp)
