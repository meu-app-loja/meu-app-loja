...
def converter_ptbr(valor):
    ...
    try:
        # Se contiver vírgula, não é número puro no Python
        if ',' not in s:
            return float(s)
    except:
        pass
    ...
    if ',' in s:
        if '.' in s:
            # Verifica se o ponto é de milhar (vem antes da vírgula)
            if s.find('.') < s.find(','):
                s = s.replace('.', '')
        s = s.replace(',', '.')
    ...
    try:
        ...
        # Tenta limpar caracteres não numéricos se ainda falhar
...
def ler_da_nuvem(nome_aba, colunas_padrao):
    ...
    try:
        ...
        # PRESERVAÇÃO DE COLUNAS: Identifica colunas existentes + as padrão
        colunas_atuais = list(df.columns)
        colunas_finais = colunas_atuais.copy()
...
def salvar_na_nuvem(nome_aba, df, colunas_padrao):
    try:
        ...
        # Prepara cópia para salvar (Preserva todas as colunas do DF)
        df_save = df.copy()
...
def unificar_produtos_por_codigo(df):
    ...
    # Preserva colunas durante unificação
