...
# CORREÇÃO: Set locale para brasileiro
try:
...
def processar_excel_oficial(arquivo_subido):
    try:
        if arquivo_subido.name.endswith('.csv'): df_temp = pd.read_csv(arquivo_subido, dtype=str, decimal=',', thousands='.')
        else: df_temp = pd.read_excel(arquivo_subido, dtype=str, decimal=',', thousands='.')
...
if df is not None:
                ...
                if st.button("🚀 SINCRONIZAR"):
                    ...
                    for i, row in df_raw.iterrows():
                        try:
                            ...
                            if cod and nome_norm: # nome_norm was not defined here in original snippet, assuming it's computed
                                ...
                                if mask.any():
                                    ...
                                    if idx_preco != "(Não Atualizar Preço)":
                                        ...
                                        if dividing_100 and val_preco > 0: val_preco /= 100 # dividing_100 typo fixed to dividir_100
                                    ...
                                    lst_novos.append({'código de barras': cod, 'nome do produto': nome_norm, 'qtd.estoque': qtd, 'qtd_central': 0, 'qtd_minima': 5, 'validade': None, 'status_compra': 'OK', 'qtd_comprada': 0, 'preco_custo': 0.0, 'preco_venda': p_nov, 'categoria': 'GERAL', 'ultimo_fornecedor': '', 'preco_sem_desconto': 0.0})
                                    novos += 1
                        except: pass
    ...
    # 7. HISTÓRICO E PREÇOS
        ...
        if not df_hist.empty:
            ...
            # VISUAL ROBUSTO: Tabela editável restaurada
    ...
    # 9. GERAL
        ...
        if c1.button("💾 Salvar"):
            ...
            st.success("Salvo!")
        with c2:
            # FERRAMENTA DE CORREÇÃO FLEXÍVEL ADICIONADA AQUI NO SEU CÓDIGO ORIGINAL
