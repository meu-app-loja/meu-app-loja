...
def converter_ptbr(valor):
    ...
    Aceita 3,19 ou 3.19 como decimais.
    """
    ...
    try:
        # Lógica Defensiva Melhorada:
        # Caso 1: Formato Brasileiro Completo (1.200,50)
        if ',' in s and '.' in s:
            s = s.replace('.', '') # Tira o ponto de milhar (1200,50)
            s = s.replace(',', '.') # Troca virgula por ponto (1200.50)
        
        # Caso 2: Apenas Vírgula (3,19) -> Vira Ponto
        elif ',' in s:
            s = s.replace(',', '.') # (3.19)
            
        # Caso 3: Apenas Ponto (3.19 ou 1.000)
        elif '.' in s:
            # Se tiver mais de um ponto, é milhar/milhão (1.000.000) -> remove
            if s.count('.') > 1:
                s = s.replace('.', '')
            else:
