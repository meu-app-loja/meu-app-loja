...
# CORREÇÃO: Set locale para brasileiro (ajuda Pandas a interpretar vírgulas corretamente)
try:
    locale.setlocale(locale.LC_ALL, 'pt_BR.UTF-8')
except:
    pass  # Se não disponível, ignora silenciosamente
...
def ler_da_nuvem(nome_aba, colunas_padrao):
    time.sleep(1) # Pausa leve para API respirar
    try:
        ...
        for col in df.columns:
            ...
            if any(x in c_low for x in ['qtd', 'preco', 'valor', 'custo', 'total', 'desconto']):
                df[col] = df[col].astype(str).apply(converter_numero_seguro)  
...
def salvar_na_nuvem(nome_aba, df, colunas_padrao):
    try:
        client = get_google_client()
        sh = client.open("loja_dados")
        # CORREÇÃO FINAL: Setar locale da planilha para 'pt_BR' para preservar vírgulas/decimais em formato brasileiro
        try:
            sh.update_locale('pt_BR')
        except AttributeError:
            # Se update_locale não estiver disponível na versão do gspread, ignora (use a versão mais recente de gspread)
            pass
        ...
        for col in df_save.columns:
            ...
            if any(x in c_low for x in ['qtd', 'preco', 'valor', 'custo', 'total', 'desconto']):
                 ...
                 # CORREÇÃO FINAL: Enviar como string com VÍRGULA (formato BR) para preservar decimais no Sheets
                 df_save[col] = df_save[col].apply(lambda x: locale.format_string("%.2f", x, grouping=True).replace('.', ',') if pd.notnull(x) else '')
          
            if pd.api.types.is_datetime64_any_dtype(df_save[col]):
                df_save[col] = df_save[col].dt.strftime('%d/%m/%Y') if pd.notnull(df_save[col]) else ''
              
        ws.update([df_save.columns.values.tolist()] + df_save.values.tolist())
        ler_da_nuvem.clear() # Limpa cache imediatamente
        time.sleep(1)  # Delay para API sincronizar
...
def processar_excel_oficial(arquivo_subido):
    try:
        ...
        if arquivo_subido.name.endswith('.csv'):
            df_temp = pd.read_csv(arquivo_subido, dtype=str, decimal=',', thousands='.')
        else:
            df_temp = pd.read_excel(arquivo_subido, dtype=str, decimal=',', thousands='.')
...
def atualizar_casa_global(nome_produto, qtd_nova_casa, novo_custo, novo_venda, nova_validade, prefixo_ignorar):
    ...
    time.sleep(2) # Pausa de segurança para evitar erro de API em chamadas seguidas
        ...
        try:
            ...
            if not df_outra.empty:
                ...
                if mask.any():
                    ...
                    ler_da_nuvem.clear()  # CORREÇÃO: Limpa cache após cada atualização global para forçar reload
        except: pass
        ...
        try:
            ...
            if not col_barras or not col_qtd: st.error("❌ Colunas 'Código de Barras' ou 'Transferir' não encontradas.")
                ...
                if st.button("🚀 PROCESSAR TRANSFERÊNCIA"):
                    ...
                    ler_da_nuvem.clear()  # CORREÇÃO: Limpa cache após transferência
                    st.rerun()  # CORREÇÃO: Força reload para atualizar menus
    ...
    if arquivo_xml:
        try:
            ...
            if st.button("✅ CONFIRMAR E SALVAR ESTOQUE E HISTÓRICO"):
                ...
                st.balloons()
                time.sleep(2)
                ler_da_nuvem.clear()  # CORREÇÃO: Limpa cache novamente após XML
                st.rerun()  # CORREÇÃO: Força reload para atualizar menus como Gôndola/Estoque
    ...
    if arquivo_base:
        if st.button("🚀 Processar e Salvar Base"):
            ...
            if sucesso:
                ...
                ler_da_nuvem.clear()  # CORREÇÃO: Limpa cache após base oficial
                st.rerun()
    ...
    if arquivo:
        try:
            ...
            if arquivo.name.endswith('.csv'):
                df_raw = pd.read_csv(arquivo, header=None, dtype=str, decimal=',', thousands='.')
            else:
                df_raw = pd.read_excel(arquivo, header=None, dtype=str, decimal=',', thousands='.')
            ...
            if st.button("🚀 SINCRONIZAR TUDO (Importar + Atualizar)"):
                ...
                ler_da_nuvem.clear()  # CORREÇÃO: Limpa cache após sincronizar
                st.rerun()  # CORREÇÃO: Força reload
        except Exception as e: st.error(f"Erro: {e}")
    ...
    with tab_imp:
        ...
        if arquivo_vendas:
            try:
                ...
                df_vendas_temp = pd.read_excel(arquivo_vendas, header=linha_titulo, dtype=str, decimal=',', thousands='.')
                ...
                if st.button("🚀 PROCESSAR VENDAS"):
                    if not df.empty:
                        ...
                        ler_da_nuvem.clear()  # CORREÇÃO: Limpa cache após vendas
                        st.rerun()  # CORREÇÃO: Força reload
            except Exception as e: st.error(f"Erro: {e}")
        ...
        if item:
            ...
            with st.form("compra"):
                ...
                if st.form_submit_button("✅ ENTRAR NO ESTOQUE"):
                    ...
                    hist = {'data': dt_full, 'produto': item, 'fornecedor': forn_compra, 'qtd': qtd, 'preco_pago': custo, 'total_gasto': qtd*custo, 'numero_nota': '', 'desconto_total_money': 0, 'preco_sem_desconto': 0}
...
# 7. HISTÓRICO & PREÇOS
    ...
    if not df_hist.empty:
        ...
        st.info("✅ Você pode editar ou **excluir** linhas (selecione a linha e aperta Delete).")
        ...
        if st.button("💾 Salvar Alterações e Exclusões", use_container_width=True):
            ...
            with st.spinner("⏳ Sincronizando preços e fornecedores do histórico para todos os menus..."):
                ...
                if alterou:
                    salvar_na_nuvem(f"{prefixo}_estoque", df, COLUNAS_VITAIS)
                    ler_da_nuvem.clear()  # CORREÇÃO: Limpa cache após sync histórico
                    st.rerun()  # CORREÇÃO: Força reload para refletir em menus
    ...
    if not df.empty:
        ...
        with c1:
            if st.button("💾 SALVAR ALTERAÇÕES GERAIS"):
                ...
                ler_da_nuvem.clear()  # CORREÇÃO: Limpa cache após edição geral
                st.rerun()
        with c2:
