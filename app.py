...
# --- FUNÇÃO DE CORREÇÃO DE VALORES (PT-BR) ---
def converter_valor_ptbr(valor):
    """
    Converte valores com vírgula (3,19) ou ponto (3.19) para float correto (3.19).
    Evita o erro de 3.19 virar 319.00.
    """
    ...
    # Se tiver vírgula, assume formato BR (decimal é vírgula)
    if ',' in s:
...
# --- LEITURA DA NUVEM (COM A CORREÇÃO DE VALORES) ---
...
def ler_da_nuvem(nome_aba, colunas_padrao):
    ...
    try:
        ...
        for col in df.columns:
            c_low = col.lower()
            # APLICA A CORREÇÃO PT-BR NOS NÚMEROS
...
# --- SALVAR NA NUVEM (LIMPA CACHE PARA NÃO APAGAR DADOS) ---
def salvar_na_nuvem(nome_aba, df, colunas_padrao):
    try:
        ...
        for col in df_save.columns:
            ...
            # Formata números para o Google Sheets (ponto ou vírgula, o Sheets entende melhor string com vírgula no BR)
            # Mas para garantir, vamos enviar como float nativo do Python, o GSheets formata.
            c_low = col.lower()
            if any(x in c_low for x in ['qtd', 'preco', 'valor', 'custo', 'total', 'desconto']):
        ...
        # IMPORTANTE: Limpa o cache para que a próxima leitura pegue os dados novos
        ler_da_nuvem.clear()
...
def unificar_produtos_por_codigo(df):
    ...
    # Aplica correção numérica antes de unificar
    cols_num = ['qtd.estoque', 'qtd_central', 'qtd_minima', 'qtd_comprada', 'preco_custo', 'preco_venda', 'preco_sem_desconto']
    for col in cols_num:
        if col in df.columns: df[col] = df[col].apply(converter_valor_ptbr)
...
def atualizar_casa_global(nome_produto, qtd_nova_casa, novo_custo, novo_venda, nova_validade, prefixo_ignorar):
    ...
    for loja in todas_lojas:
        ...
        # Força leitura nova para não sobrescrever com dados velhos
        ler_da_nuvem.clear()
    ...
    if itens_custom:
        for it in itens_custom:
            try:
                ...
                qtd = converter_valor_ptbr(it.find("Quantidade").text)
                valor = converter_valor_ptbr(it.find("ValorPagoFinal").text)
                ean = it.find("CodigoBarras").text
                desc = 0.0
                if it.find("ValorDesconto") is not None: desc = converter_valor_ptbr(it.find("ValorDesconto").text)
        ...
        # 3. SE NÃO ACHOU, TENTA FORMATO OFICIAL (NFe)
            ...
            try:
                ...
                if prod:
                    ...
                    for info in prod:
                        ...
                        elif t == 'qCom': qCom = converter_valor_ptbr(info.text)
                        elif t == 'vProd': vProd = converter_valor_ptbr(info.text)
                        elif t == 'vDesc': vDesc = converter_valor_ptbr(info.text)
                    if qCom > 0:
...
# ==============================================================================
# 🚀 INÍCIO DO APP (INTERFACE ORIGINAL RESTAURADA)
# ==============================================================================
                ...
                if not col_barras or not col_qtd: st.error("❌ Colunas 'Código de Barras' ou 'Transferir' não encontradas.")
                    ...
                    if st.button("🚀 PROCESSAR TRANSFERÊNCIA"):
                        ...
                        for i, row in df_pick.iterrows():
                            ...
                            qtd_pick = converter_valor_ptbr(row[col_qtd])
                            if qtd_pick > 0:
                ...
                if st.button("🚀 SINCRONIZAR TUDO (Importar + Atualizar)"):
                    ...
                    for i in range(start_row, total_linhas):
                        try:
                            ...
                            qtd = converter_valor_ptbr(df_raw.iloc[i, idx_qtd])
                            ...
                            if cod and nome_norm and pd.notnull(qtd):
                                ...
                                if mask.any():
                                    ...
                                    if idx_preco != "(Não Atualizar Preço)":
                                        val_preco = converter_valor_ptbr(df_raw.iloc[i, idx_preco])
                                        if pd.notnull(val_preco): df.loc[mask, 'preco_venda'] = val_preco
                                    alt += 1
                                else:
                                    novo_preco_venda = 0.0
                                    if idx_preco != "(Não Atualizar Preço)":
                                        val_p = converter_valor_ptbr(df_raw.iloc[i, idx_preco])
                    ...
                    if st.button("🚀 PROCESSAR VENDAS"):
                        if not df.empty:
                            ...
                            for i, row in df_vendas_temp.iterrows():
                                ...
                                qtd = converter_valor_ptbr(row[col_qtd])
                                try:
