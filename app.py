...
# ✅ NOVO: input de dinheiro que aceita 3,19 (evita virar 319)
def money_input_ptbr(label, key, value=0.0, help=None):
    """
    Campo monetário que aceita 3,19 ou 3.19.
    Evita o bug do st.number_input que pode transformar 3,19 em 319 no navegador.
    """
    try:
        txt_default = format_br(float(value))
    except:
        txt_default = "0,00"

    txt = st.text_input(label, value=txt_default, key=key, help=help, placeholder="Ex: 3,19")
...
def salvar_na_nuvem(nome_aba, df, colunas_padrao):
    try:
        ...
        # Prepara cópia para salvar (transforma datas em texto para o Excel não reclamar)
        df_save = garantir_integridade_colunas(df.copy(), colunas_padrao)

        for col in df_save.columns:
            if pd.api.types.is_datetime64_any_dtype(df_save[col]):
                df_save[col] = df_save[col].dt.strftime('%Y-%m-%d')  # Formato ISO para consistência
...
def unificar_produtos_por_codigo(df):
    ...
    for cod, grupo in com_codigo.groupby('código de barras'):
        if len(grupo) > 1:
            ...
            # mantém último fornecedor se existir em algum
            ult = ""
            for v in grupo.get('ultimo_fornecedor', pd.Series([])).astype(str).tolist():
                v = v.strip()
                if v:
                    ult = v
            if ult:
                base_ref['ultimo_fornecedor'] = ult
...
# ✅ CORRIGIDO: match por nome normalizado (não falha por detalhe)
def atualizar_casa_global(nome_produto, qtd_nova_casa, novo_custo, novo_venda, nova_validade, prefixo_ignorar):
    ...
    for loja in todas_lojas:
        ...
        if mask.any():
            ...
            df_outra.at[idx, 'qtd_central'] = converter_ptbr(qtd_nova_casa)
            if novo_custo is not None: 
                df_outra.at[idx, 'preco_custo'] = converter_ptbr(novo_custo)
            if novo_venda is not None: 
                df_outra.at[idx, 'preco_venda'] = converter_ptbr(novo_venda)
...
def ler_xml_nfe(arquivo_xml, df_referencia):
    ...
    if info_custom is not None:
        ...
        # ✅ CORRIGIDO: pega número e fornecedor do emitente, não "qualquer xNome"
        dados_nota = {'numero': 'S/N', 'fornecedor': 'IMPORTADO', 'data': datetime.now(), 'itens': []}
        ...
        if arquivo_xml:
            try:
                ...
                if st.button("✅ CONFIRMAR E SALVAR ESTOQUE"):
                    ...
                    # ✅ CORRIGIDO: unifica duplicados por código antes de salvar
                ...
                with tab_acao:
                    ...
                    if opcao_busca != "":
                        ...
                        with st.expander("🛠️ Ajustes Manuais (Completo)"):
                            ...
                            # ✅ CORRIGIDO: aceitar 3,19
                            n_custo = money_input_ptbr("Preço Custo:", key=f"gond_custo_{idx}", value=float(df.at[idx, 'preco_custo']))
                            n_venda = money_input_ptbr("Preço Venda:", key=f"gond_venda_{idx}", value=float(df.at[idx, 'preco_venda']))
                            c1, c2 = st.columns(2)
        ...
        if not pen.empty:
            ...
            if item:
                ...
                with st.form("compra"):
                    ...
                    # ✅ CORRIGIDO: aceitar 3,19
                    custo = money_input_ptbr("Preço Pago (UN):", key=f"compra_custo_{idx}", value=float(df.at[idx, 'preco_custo']))
                    venda = money_input_ptbr("Novo Preço Venda:", key=f"compra_venda_{idx}", value=float(df.at[idx, 'preco_venda']))
                ...
                if usar_modo_mobile:
                    ...
                    for idx, row in df_show.iterrows():
                        with st.container(border=True):
                            ...
                            # ✅ CORRIGIDO: aceitar 3,19
                            novo_custo = money_input_ptbr("Custo:", key=f"c_{idx}", value=float(row['preco_custo']))
                ...
                if prod_opcao:
                    ...
                    if mask.any():
                        ...
                        with st.form("edit_estoque_casa_full"):
                            ...
                            # ✅ CORRIGIDO: aceitar 3,19
                            novo_custo = money_input_ptbr("Custo:", key=f"casa_custo_{idx_prod}", value=float(df.at[idx_prod, 'preco_custo']))
                            novo_venda = money_input_ptbr("Venda:", key=f"casa_venda_{idx_prod}", value=float(df.at[idx_prod, 'preco_venda']))
