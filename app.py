    ...
    for item in dados_nota['itens']:
        if item['ean'] in ['SEM GTIN', '', 'None', 'NAN'] and lista_nomes_ref:
            ...
            if melhor: 
                ...
                item['match_tipo'] = tipo # AQUI: Guarda o tipo para mostrar no visual
    return dados_nota
...
if df is not None:
    ...
    # 1.5 PICKLIST
        ...
        if arquivo_pick:
            try:
                # CORREÇÃO: dtype=str para ler numeros como texto e não perder ponto/virgula
            ...
            try:
                ...
                if st.button("✅ CONFIRMAR E SALVAR ESTOQUE"):
                    ...
                    for i, item in enumerate(dados['itens']):
                        ...
                        # ATUALIZA A CASA (COM PROTEÇÃO TRY/EXCEPT)
                        try:
                    ...
                    ler_da_nuvem.clear() # CACHE LIMPO = GONDOLA ATUALIZADA
    ...
    # 3. SINCRONIZAR
        ...
        if arquivo:
            try:
                ...
                # VISUAL RESTAURADO: MOSTRA A TABELA PARA IDENTIFICAR COLUNAS
                ...
                if st.button("🚀 SINCRONIZAR TUDO (Importar + Atualizar)"):
                    ...
                    st.success(f"✅ Sucesso! {alt} atualizados e {novos} novos.")
    ...
    # 5. GÔNDOLA
        ...
        if df.empty: st.warning("Cadastre produtos.")
            ...
            if usar_modo_mobile:
                ...
                with tab_acao:
                    # RESTAURADO: BUSCA POR CÓDIGO E NOME
    ...
    # 7. HISTÓRICO & PREÇOS (VISUAL EDITÁVEL RESTAURADO)
        ...
        if not df.empty:
            ...
            with c1:
                if st.button("💾 SALVAR ALTERAÇÕES GERAIS"):
                    ...
                    st.rerun()
            with c2:
                # SELETOR DE CORREÇÃO FLEXÍVEL ADICIONADO AQUI NO LAYOUT ORIGINAL
