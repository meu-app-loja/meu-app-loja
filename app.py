...
# --- FUNÇÃO DE SEGURANÇA NUMÉRICA (NOVA) ---
def converter_numero_seguro(valor):
    """Garante que 3,30 seja 3.30 e não 330"""
    ...
    try:
        ...
        # Se tem virgula (ex: 3,30), troca por ponto
        if ',' in s_valor:
...
def ler_da_nuvem(nome_aba, colunas_padrao):
    ...
    try:
        ...
        # APLICA A CORREÇÃO NUMÉRICA SEGURA
...
def salvar_na_nuvem(nome_aba, df, colunas_padrao):
    try:
        ...
        for col in df_save.columns:
            c_low = col.lower()
            # Garante que vai como número correto para o Google
            if any(x in c_low for x in ['qtd', 'preco', 'valor', 'custo', 'total', 'desconto']):
...
def atualizar_casa_global(nome_produto, qtd_nova_casa, novo_custo, novo_venda, nova_validade, prefixo_ignorar):
    ...
    # Pausa removida para performance, ou recolocada se der erro API
    time.sleep(0.5) 
    for loja in todas_lojas:
        if loja == prefixo_ignorar: continue
        # Bloco try/except individual para não travar o app se uma loja falhar
        try:
            client = get_google_client()
            sh = client.open("loja_dados")
...
if df is not None:
    ...
    # 1.5 PICKLIST
        ...
        if arquivo_pick:
            try:
                # CORREÇÃO CRUCIAL: Ler como texto para manter formatação original
        ...
        if arquivo_base:
            if st.button("🚀 Processar e Salvar Base"):
                ...
                if sucesso:
                    ...
                    st.rerun()
        st.divider()
        if st.button("🗑️ ZERAR TUDO (PERIGO)"):
