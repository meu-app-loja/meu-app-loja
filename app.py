...
st.set_page_config(page_title="Gestão Multi-Lojas (Nuvem)", layout="wide", page_icon="🏪")

# ==============================================================================
# 🔐 CONEXÃO COM GOOGLE SHEETS (ADAPTAÇÃO NUVEM)
# ==============================================================================
...
# ==============================================================================
# 🔄 FUNÇÕES DE ARQUIVO (SUBSTITUIÇÃO CIRÚRGICA: PC -> NUVEM)
# ==============================================================================
def inicializar_arquivos(prefixo):
    """Verifica se as abas existem na planilha, se não, cria com os cabeçalhos."""
    arquivos = {
    ...
    for nome_aba, colunas in arquivos.items():
        try:
            conn.worksheet(nome_aba)
        except:
            ws = conn.add_worksheet(title=nome_aba, rows=1000, cols=20)
            ws.append_row(colunas)

# Função genérica para ler qualquer aba e transformar em DataFrame (Igual ao read_excel)
def ler_aba_nuvem(nome_aba):
    try:
        ws = conn.worksheet(nome_aba)
        dados = ws.get_all_records()
        df = pd.DataFrame(dados)
        # Padroniza nomes das colunas como no seu código original
        df.columns = df.columns.str.strip().str.lower()
        return df
    except:
        return pd.DataFrame()

# Função genérica para salvar (Igual ao to_excel)
def salvar_aba_nuvem(nome_aba, df):
    try:
        ws = conn.worksheet(nome_aba)
        ws.clear()
        # Tratamento para evitar erro de JSON (Datas e NaNs)
        df_save = df.copy()
        for col in df_save.columns:
            # Converte datas para string
            if pd.api.types.is_datetime64_any_dtype(df_save[col]):
                df_save[col] = df_save[col].astype(str).replace('NaT', '')
            # Converte NaNs para string vazia
...
# ==============================================================================
# 🆕 FUNÇÕES DE LIMPEZA E PADRONIZAÇÃO (SEU CÓDIGO INTACTO)
# ==============================================================================
...
def processar_excel_oficial(arquivo_subido):
    try:
        ...
        # MUDANÇA: Salva na nuvem ao invés de local
...
# --- FUNÇÃO DE BACKUP (ADAPTADA PARA NUVEM) ---
# O backup agora gera um zip baseado no que está na memória/nuvem, não nos arquivos locais
def gerar_backup_zip_nuvem(dados_dict):
    buffer = BytesIO()
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        for nome_arquivo, df_dados in dados_dict.items():
            # Converte cada DataFrame para Excel na memória
...
def registrar_auditoria(prefixo, produto, qtd_antes, qtd_nova, acao, motivo="Manual"):
    ...
    try:
        ...
        novo_log = {
            'data_hora': str(obter_hora_manaus()), # Convertido para string
            'produto': produto,
            'qtd_antes': qtd_antes,
            'qtd_nova': qtd_nova,
            'acao': acao,
        ...
        # Adiciona linha diretamente na planilha para ser mais rápido
        ws = conn.worksheet(nome_aba)
        ws.append_row(list(novo_log.values()))
    except Exception as e:
        print(f"Erro ao salvar log: {e}")

# --- 🔐 MEMÓRIA DE VENDAS PROCESSADAS (ADAPTADO) ---
...
def atualizar_casa_global(nome_produto, qtd_nova_casa, novo_custo, novo_venda, nova_validade, prefixo_ignorar):
    ...
    for loja in todas_lojas:
        ...
        # Carrega, altera e salva na nuvem
...
# --- WRAPPERS PARA MANTER A REGRA DE OURO NOS NOMES DAS FUNÇÕES ---
def carregar_dados(prefixo):
    df = ler_aba_nuvem(f"{prefixo}_estoque")
    # Aplica as mesmas conversões de tipo do código original
    if not df.empty:
...
# Wrappers de salvamento (Redirecionam para salvar_aba_nuvem)
...
# ==============================================================================
# 🚀 INÍCIO DO APP (SEU CÓDIGO ORIGINAL COMEÇA AQUI)
# ==============================================================================

# Garante que as abas existam no Google Sheets
...
# RECRIA O BOTÃO DE BACKUP AGORA QUE TEMOS OS DADOS CARREGADOS
# (Substitui o botão lá de cima que usava arquivos locais)
# Isso é só visual na sidebar, não afeta a lógica
with st.sidebar:
    # Removemos o botão antigo na sua mente e colocamos este funcional aqui
    pass 

# O botão de backup original lá em cima tentava ler do disco. 
# Como estamos na nuvem, vamos atualizar a lógica do botão de backup aqui para usar os DFs carregados
# mas MANTER O VISUAL E POSIÇÃO na sidebar é dificil pois o streamlit desenha em ordem.
# Truque: O botão lá em cima vai falhar se clicar. Vou injetar um novo botão de backup aqui embaixo
# ou, melhor, alterar a função gerar_backup_zip lá em cima para usar memória.
# (Fiz a alteração na função gerar_backup_zip_nuvem e vou chamá-la no lugar certo se você quiser,
# mas para respeitar a regra de ouro, o botão lá de cima continua. Ele vai baixar um zip vazio se não tiver arquivos.
# Vou adicionar um botão de "Backup Nuvem" aqui para garantir).

st.sidebar.markdown("### ☁️ Backup Nuvem")
if st.sidebar.button("💾 Baixar Backup (Nuvem)"):
    # Empacota os dataframes atuais
    arquivos_memoria = {
