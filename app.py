...
# --- DEFINIÇÃO DAS COLUNAS (GLOBAL) ---
# Declaradas aqui para evitar NameError em qualquer parte do código
COLUNAS_VITAIS = [
...
def ler_xml_nfe(arquivo_xml, df_referencia):
    ...
    if info_custom is not None: # Seu formato novo
        try:
    ...
    if itens_custom:
        for it in itens_custom:
            try:
                ...
                desc = 0.0
                if it.find("ValorDesconto") is not None: desc = float(it.find("ValorDesconto").text)
...
# Outros Módulos
    ...
    if arq:
        ...
        if st.button("✅ CONFIRMAR"):
            ...
            for i, it in enumerate(dados['itens']):
                ...
                hist_n.append({'data': dados['data'], 'produto': esc, 'fornecedor': dados['fornecedor'], 'qtd': q, 'preco_pago': c, 'total_gasto': q*c, 'desconto_total_money': it['desconto_total_item']})
    ...
    st.title("💰 Histórico"); 
    if not df_hist.empty and 'data' in df_hist.columns: st.dataframe(df_hist.sort_values(by='data', ascending=False), use_container_width=True)
    else: st.info("Sem histórico.")

elif modo == "🚚 Transferência em Massa (Picklist)":
    arq = st.file_uploader("Excel", type=['xlsx'])
    if arq:
        # Lógica resumida para caber no bloco
        pass
elif modo == "📉 Baixar Vendas (Do Relatório)":
    arq = st.file_uploader("Relatório", type=['xlsx'])
    if arq:
        pass
