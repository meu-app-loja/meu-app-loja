...
# --- FUNÇÃO DE LIMPEZA E CONVERSÃO DE NÚMEROS (CORRIGIDA PARA CENTAVOS) ---
def converter_ptbr(valor):
    """
    Força a conversão correta de decimais, priorizando o formato 7.99 ou 7,99.
    """
    ...
    try:
        # Caso 1: Formato Brasileiro Completo (ex: 1.200,50) -> Tem ponto E vírgula
        if ',' in s and '.' in s:
            s = s.replace('.', '') # Tira o ponto de milhar (1200,50)
            s = s.replace(',', '.') # Troca virgula por ponto (1200.50)
        
        # Caso 2: Apenas Vírgula (ex: 7,99 ou 1200,50) -> Vira Ponto
        elif ',' in s:
            s = s.replace(',', '.') 
            
        # Caso 3: Apenas Ponto (ex: 7.99 ou 1.000)
        elif '.' in s:
            # Se tiver mais de um ponto (1.000.000), é milhar
            if s.count('.') > 1:
                s = s.replace('.', '')
            else:
                # Se tem apenas 1 ponto, vamos analisar:
                # Se for algo como 7.99, mantém o ponto.
                # Se for 799.00 (formato excel as vezes), mantém.
                pass 
...
# --- SALVAR NA NUVEM (CORRIGIDO PARA SALVAR HORA E PREÇO) ---
    ...
    try:
        ...
        for col in df_save.columns:
            if pd.api.types.is_datetime64_any_dtype(df_save[col]):
                # Se for Validade, só Data
                if 'validade' in col.lower():
                    df_save[col] = df_save[col].dt.strftime('%Y-%m-%d')
                # Se for Histórico/Movimentação, Data e Hora
                else:
            ...
            if not df.empty:
                ...
                if p_op:
                    ...
                    with st.form("manual_casa"):
                        ...
                        # CORREÇÃO DA HORA E DATA
                        dt_r = c_dt.date_input("Data:", agora_am())
                        # STEP 60 PERMITE MINUTOS EXATOS
                        ...
                        if st.form_submit_button("💾 SALVAR MOVIMENTAÇÃO"):
                            # AQUI ESTAVA O ERRO: USAVA AGORA() EM VEZ DO INPUT
                            ...
                            if acao.startswith("Somar") and qtd_in > 0:
                                ...
                                # SALVA NO HISTÓRICO COM A DATA/HORA ESCOLHIDA MANUALMENTE
                            ...
                            st.success(f"Salvo com data: {dt_full.strftime('%d/%m/%Y %H:%M')}"); st.rerun()
